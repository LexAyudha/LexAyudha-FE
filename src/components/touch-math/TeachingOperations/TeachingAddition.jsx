...
const TeachingAddition = () => {
  ...
  const startTeaching = () => {
    ...
    pronounceNumber(num1, () => {
      setInterval(() => {
        setHighlightedTouchPoint(touchPointIndex);
        touchPointIndex += 1;
        if (touchPointIndex >= num1) {
          clearInterval();
        }
      }, 1000);

      setTimeout(() => {
        const plusUtterance = new SpeechSynthesisUtterance('Plus');
        window.speechSynthesis.speak(plusUtterance);
      }, 1000);
...
export default TeachingAddition;
