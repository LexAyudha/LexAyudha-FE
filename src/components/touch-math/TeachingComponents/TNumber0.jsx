...
const Number0 = () => {
  ...
  useEffect(() => {
    ...
    const number = localStorage.getItem("selectedNumber") ?? 0;
    setCurrentNumberIndex(number);
    const synth = window.speechSynthesis;
    const getAvailableVoices = () => {
      const availableVoices = synth.getVoices();
      const female = availableVoices.find(
  ...
  const speakNumber = (number) => {
    if ("speechSynthesis" in window && femaleVoice) {
      ...
      console.warn(
        "Text-to-Speech not supported or female voice is not loaded."
      );
    }
  };
      ...
      <>
        <div
          ...
          style={{
            ...
            left: "45.45%",
            transform: "translate(-50%, -50%)",
            backgroundColor: "var(--main-ring-color)",
          }}
        ></div>
        <div
    ...
    <div className="la-container relative  overflow-x-hidden overflow-y-hidden flex flex-col items-center justify-center h-screen">
      ...
      <div className=" w-full justify-center items-center h-fit relative">
        <div className="flex flex-col items-center justify-center">
          {/* Header */}
          <h1 className="w-fit">
            Let's Learn Number {numberList[currentNumberIndex]}
          </h1>
        ...
        <div>
          <button
            className={`next-button ${
              currentNumberIndex + 1 < numberList.length ? "flex" : "hidden"
            }`}
          ...
          <button
            className={`next-button ${
              currentNumberIndex + 1 < numberList.length ? "hidden" : "flex"
            }`}
      ...
      <div
        className={` absolute z-50  right-[100px] top-1/2 transform -translate-y-1/2 rounded-[20px_0px_0px_20px] w-[300px] h-[550px] primary-color-bg transition-transform duration-300 ${
          isPanelOpen ? "translate-x-[100px]" : "translate-x-[390px]"
        }`}
      >
        <div
          id="side_panel"
          className={`cursor-pointer flex items-center justify-center absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-[20px] rounded-[5px] primary-color-bg px-[10px] py-[20px] transition-transform duration-300 ${
